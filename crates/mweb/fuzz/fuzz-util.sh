#!/usr/bin/env bash
# Shared helpers for the bdk_mweb fuzz scripts.
# Mirrors rust-litecoin/fuzz/fuzz-util.sh so both harnesses are driven the same way.

REPO_DIR=$(git rev-parse --show-toplevel)
FUZZ_DIR="$REPO_DIR/crates/mweb/fuzz"
...
targetFileToHFuzzInputArg() {
  ...
  if [ -d "hfuzz_input/$dirName" ]; then
    echo "HFUZZ_INPUT_ARGS=-f${FUZZ_DIR}/hfuzz_input/${dirName}/input\\"
  fi
}
