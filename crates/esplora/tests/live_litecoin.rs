//! Live sync tests against litecoinspace.org.
//!
...
//! just test-live
//! # or
//! cargo test -p bdk_esplora --test live_litecoin -- --ignored --nocapture
//! ```
...
#[test]
#[ignore = "live network: litecoinspace.org"]
fn esplora_syncs_mainnet_p2pkh_fixture() {
    sync_and_assert(P2PKH_ADDR);
}

#[test]
#[ignore = "live network: litecoinspace.org"]
fn esplora_syncs_mainnet_p2sh_fixture() {
...
fn sync_and_assert(address: &str) {
    ...
    let client = Builder::new(ESPLORA_MAINNET).build_blocking();
    ...
    let response = client
        .sync(request, 1)
        .unwrap_or_else(|e| panic!("sync against {ESPLORA_MAINNET} failed: {e}"));
